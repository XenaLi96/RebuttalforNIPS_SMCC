...
\textit{\textbf{Table 2. Full-panel Visium HD transfer before calibration.}
TM is the source mean; its gene correlation is undefined.}\\[3pt]
...
Full-panel Gene Pearson is low; UNI2-h improves F1 but not Cell Pearson over
the mean. We retain this negative result and do not conflate
sample/processing/composition shift with the easier spatial-interpolation
endpoint.

\textbf{A2 (Reviewer-inspired calibration).}\enspace We thank the reviewer for
suggesting metadata/spatial controls. On identical cells/genes we fit
coordinate-only, segmentation-metadata-only, and combined baselines. Metadata
include CellViT type/confidence, polygon/bbox morphology, status, and edge flag;
...
\begin{itemize}
\item Segmentation metadata improve Gene Pearson in 8/8 organs and raise
organ-macro Gene/Cell Pearson from 0.0151/0.2036 to 0.0399/0.2344.
Coordinate-only and combined reach Gene Pearson -0.0023 and 0.0316,
respectively.
\item F1 is lower than UNI2-h (0.0432 versus 0.0815). We will therefore use
this as the principal non-image \emph{correlation-calibration} baseline
alongside the mean, not claim universal superiority. Age/sex/disease are not
identifiable from one source/target pair per organ.
\end{itemize}
...
\textbf{A4 (Boundary).}\enspace Marker overlap and expression-derived strata
provide aggregate biological calibration, not independent labels. Together,
the controls demonstrate context dependence and motivate
patient/platform-robust methods rather than unrestricted cell-wise recovery.
