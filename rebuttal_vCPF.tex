...
\textbf{A3 (Visium HD transfer).}\enspace Eight
source-sample-to-target-sample organ pairs give:
...
\begin{itemize}
\item UNI2-h improves F1 but not Cell Pearson over the mean baseline; this
transfer failure is intentionally not hidden.
\item We will report organ-macro and per-organ values, separate Xenium from
Visium HD, and avoid cell-weighted pooled claims.
\end{itemize}
...
\textbf{A1 (Interpretation).}
\begin{itemize}
\item Low full-panel correlations show that histology is not a replacement for
molecular measurement.
\item They combine sparse or weakly morphology-linked genes with sample,
composition, and acquisition shifts.
\item A restricted training-selected HVG endpoint is easier and must not be
conflated with all-gene transfer.
\end{itemize}

\textbf{A2 (Biological calibration).}\enspace The 30-sample spatially held-out
predictions give:
...
\textbf{A3 (Hard-transfer boundary).}
\begin{itemize}
...
\item This negative result demonstrates context dependence and motivates stronger
